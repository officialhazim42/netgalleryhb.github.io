...
const closeChatBot = document.getElementById('closeChat');
...
const botResponses = {
  'printing': {
    keywords: ['print', 'printing', 'document', 'color', 'black', 'a4', 'a3'],
    response: '🖨️ We offer professional printing services:\n• A4/A3 Printing (Color & B&W)\n• Document Binding & Lamination\n• High-Quality Color Prints\nVisit us or call +91 80824 22129 for details!'
  },
  'phone': {
    keywords: ['phone', 'cover', 'mobile', 'case', 'custom'],
    response: '📱 Custom Phone Covers Available:\n• All Mobile Models Supported\n• Vibrant, High-Quality Printing\n• Quick Turnaround Time\n• Competitive Pricing\nContact us today!'
  },
  'government': {
    keywords: ['government', 'govt', 'services', 'scheme', 'passport', 'aadhaar', 'pan', 'voter', 'driving', 'license'],
    response: '🏛️ Government Services:\n• PM-SYM Scheme Registration\n• Bank Account Opening\n• Voter ID & Driving License\n• PAN & ABHA Cards\n• E-Shram & APAAR Registration\n\nWe handle all documentation!'
  },
  'booking': {
    keywords: ['booking', 'appointment', 'reserve', 'schedule', 'time', 'date'],
    response: '📅 Easy Booking Process:\n• Fill the form on our website\n• Or WhatsApp us directly\n• We confirm within minutes\n• Operating Hours: Mon-Sat 10 AM - 7 PM\n\nLet\'s book your service now!'
  },
  'location': {
    keywords: ['location', 'address', 'where', 'ganderbal', 'sahipora'],
    response: '📍 Our Location:\nNet Gallery HB\nSahipora, Ganderbal\nJammu & Kashmir, 191201\n\nView on Google Maps in the Contact section!'
  },
  'hours': {
    keywords: ['hours', 'timing', 'open', 'close', 'available', 'time'],
    response: '🕐 Operating Hours:\nMonday - Saturday\n10:00 AM - 7:00 PM\n\nSunday: Closed\n\nContact us anytime via WhatsApp!'
  },
  'contact': {
    keywords: ['contact', 'phone', 'call', 'whatsapp', 'email', 'message'],
    response: '📞 Contact Us:\n• WhatsApp: +91 80824 22129\n• Instagram: @netgalleryhb\n• YouTube: @NetGalleryHb\n• Facebook: Net Gallery Hb\n\nReply within minutes!'
  },
  'price': {
    keywords: ['price', 'cost', 'rate', 'charge', 'fee', 'amount'],
    response: '💰 Pricing Information:\nOur rates are competitive and transparent:\n• Printing: Starting from ₹2-10/page\n• Phone Covers: ₹300-500\n• Government Services: Variable\n\nContact us for exact quotes!'
  },
  'default': {
    response: '👋 Thanks for asking! I\'m here to help with:\n• 🖨️ Printing Services\n• 📱 Custom Phone Covers\n• 🏛️ Government Services\n• 📅 Bookings\n• 📍 Location & Hours\n\nWhat can I help you with?'
  }
};
...
function findBotResponse(userMessage) {
  ...
  for (const key in botResponses) {
    ...
    const keywords = botResponses[key].keywords;
    if (keywords.some(keyword => msg.includes(keyword))) {
      return botResponses[key].response;
    }
  }
...
function addChatbotMessage(text, isUser = false) {
  ...
  small.textContent = `Today at ${now}`;
...
function sendChatbotMessage() {
  ...
  setTimeout(() => {
    const response = findBotResponse(message);
...
sendChat.addEventListener('click', sendChatbotMessage);
